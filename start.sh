...
export MB_DB_USER=`echo $DATABASE_URL | sed -n 's/\([A-z0-9]*\):.*/\1/p'`
export MB_DB_PASS=`echo $DATABASE_URL | sed -n 's/.*:\([A-z0-9]*\)@.*/\1/p'`
export MB_DB_HOST=`echo $DATABASE_URL | sed -n 's/.*@tcp(\([A-z0-9\.\-]*\).*/\1/p'`
export MB_DB_PORT=`echo $DATABASE_URL | sed -n 's/.*@tcp([A-z0-9\.\-]*:\([0-9]*\)).*/\1/p'`
export MB_DB_DBNAME=`echo $DATABASE_URL | sed -n 's/.*\/\([A-z0-9]*\)$/\1/p'`

echo "Starting metabase... Listening on 3000"
